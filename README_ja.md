...
- `get_shader_source` - RenderDoc がテキストの元ソースを保持している場合に shader ソースを取得
...
原子ツールの上で複数サービスをオーケストレーションする多次元クエリで、素早く全体像を把握できます。
...
詳細な使用例は [MCP Tools Reference](docs/tools.md) を参照してください。
